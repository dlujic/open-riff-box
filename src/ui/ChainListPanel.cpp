...
juce::Colour ChainListPanel::getColourForEffect(const juce::String& name)
{
    ...
    if (name == "Tremolo")            return Theme::Colours::modulation;
...
void ChainListPanel::rebuildFromChain()
{
    ...
    for (int i = 0; i < effectChain.getNumEffects() && numVisualSlots < kNumVisualSlots; ++i)
    {
        ...
        // Skip Flanger, Phaser, Vibrato, Tremolo - merged into the single Chorus visual row
...
{
    activeModulationEngine = juce::jlimit(0, 4, engine);
    refreshBypassStates();
}
